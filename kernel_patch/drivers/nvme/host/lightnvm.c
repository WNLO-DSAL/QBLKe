...
static void *nvme_nvm_alloc_cmd(struct nvm_dev *dev, gfp_t flags)
{
	return kmem_cache_alloc(dev->cmd_kmemcache, flags);
	//kzalloc(sizeof(struct nvme_nvm_command), flags);
}

static void nvme_nvm_free_cmd(struct nvm_dev *dev, void *p)
{
	kmem_cache_free(dev->cmd_kmemcache, p);
	//kfree(p);
}
static void nvme_nvm_end_io(struct request *rq, blk_status_t status)
{
	struct nvm_rq *rqd = rq->end_io_data;
	struct nvm_dev *dev = rqd->dev->parent;

	rqd->ppa_status = le64_to_cpu(nvme_req(rq)->result.u64);
	rqd->error = nvme_req(rq)->status;
	nvm_end_io(rqd);

	nvme_nvm_free_cmd(dev, nvme_req(rq)->cmd);
...
static struct request *nvme_nvm_alloc_request_nowait(struct request_queue *q,
					      struct nvm_rq *rqd,
					      struct nvme_nvm_command *cmd)
{
	struct nvme_ns *ns = q->queuedata;
	...
	rq = nvme_alloc_request(q, (struct nvme_command *)cmd, BLK_MQ_REQ_NOWAIT, NVME_QID_ANY);
	if (IS_ERR(rq))
		return rq;
...
static int nvme_nvm_submit_io_nowait(struct nvm_dev *dev, struct nvm_rq *rqd)
{
	...
	cmd = nvme_nvm_alloc_cmd(dev, GFP_ATOMIC);
	if (!cmd)
		return -EAGAIN;

	rq = nvme_nvm_alloc_request_nowait(q, rqd, cmd);
	if (IS_ERR(rq)) {
		kfree(cmd);
		return PTR_ERR(rq);
	}
...
static int nvme_nvm_submit_io(struct nvm_dev *dev, struct nvm_rq *rqd)
{
	...
	cmd = nvme_nvm_alloc_cmd(dev, GFP_KERNEL);
	if (!cmd)
		return -ENOMEM;
...
static struct nvm_dev_ops nvme_nvm_dev_ops = {
	...
	.submit_io_nowait		= nvme_nvm_submit_io_nowait,
...
int nvme_nvm_register(struct nvme_ns *ns, char *disk_name, int node)
{
	...
	snprintf(cachename, 32, "%s_cmd_kmemcache", disk_name);
	dev->cmd_kmemcache = kmem_cache_create(cachename,
					sizeof(struct nvme_nvm_command), 0, 0, NULL);
...
{
	struct kmem_cache *cmd_kmemcache = ns->ndev->cmd_kmemcache;

	nvm_unregister(ns->ndev);
	kmem_cache_destroy(cmd_kmemcache);
}
