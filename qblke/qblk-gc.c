...
static int qblk_perch_gc_activated(struct qblk *qblk, int chnl)
{
	...
	spin_lock_irqsave(&qblk->gc_active_lock, flag);
	ret = test_bit(chnl, qblk->gc_active);
	spin_unlock_irqrestore(&qblk->gc_active_lock, flag);
	return ret;
}
...
static int qblk_gc_free_full_lines(struct qblk_gc *gc, struct ch_info *chi)
{
	...
	do {
		...
		qblk_debug_log(gc->qblk, QBLK_GC_FULL, gc->chnl,
							line->id, 0);
...
#ifdef QBLKe_DEBUG
	atomic_add(1, &qblk->rl.gc_read_rq);
#endif
...
retry:
	...
	gc->w_entries++;
#ifdef QBLKe_DEBUG
	atomic_add(1, &qblk->rl.gc_read_queued);
#endif
...
static void qblk_gc_line(struct work_struct *work)
{
	...
	//pr_notice("%s, ch[%d] line[%d] vsc[%d]\n", __func__,
	//				gc->chnl, line->id, qblk_line_vsc(line));
...
next_rq:
	...
	} while (nr_secs < QBLK_GC_RQ_MAX_SIZE);
...
#ifdef QBLKe_DEBUG
	atomic_add(1, &qblk->rl.gc_create_rq);
#endif
...
next_gc_group:
	...
	if (likely(gc_group < QBLK_GC_NR_LISTS)) {
		...
		//pr_notice("%s, chnl %u victim %u\n", __func__, chnl, line->id);
		...
		qblk_debug_log(qblk, QBLK_GC_MIGRATE,
						chnl, id, vsc);
...
void qblk_gc_should_start(struct qblk_per_chnl_rl *rl)
{
	...
	if (unlikely(!qblk->per_channel_gc))
		return;

	gc = &qblk->per_channel_gc[chnl];
