			...
			unsigned int valid_secs, unsigned int rb_count)
{
	...
	for (i = 0; i < nr_secs;
		...
		if (i < valid_secs) {
			...
			rb = qblk_get_rb_by_cpuid(qblk, rb_count);
			w_ctx = qblk_rb_w_ctx(qblk, rb,
				qblk_rb_wrap_pos(rb, sentry + i));
			flags = READ_ONCE(w_ctx->flags);

			if (ADDR_EMPTY == w_ctx->lba) {
				//pr_notice("%s, dummyToInvalid i[%d]\n",
				//			__func__, i);
				w_ctx->ppa.ppa = ADDR_EMPTY;
				goto invalidate;
			}
			kref_get(&line->ref);
...
invalidate:
			//pr_notice("%s, invalidate, i=%d, ofset_in_line=%llu\n",
			//			__func__, i, offset_in_line);
	...
	qblk_mark_rq_luns(qblk, ppa_list, lun_bitmap);
}
		    ...
		    unsigned int rb_count)
{
	...
	for (i = 0; i < rqd->nr_ppas; i += min) {
		...
		spin_lock_irqsave(&e_line->lock, flags);
		if (!test_bit(erase_lun, e_line->erase_bitmap)) {
			...
			*erase_ppa = rqd->ppa_list[i];
			...
			spin_unlock_irqrestore(&e_line->lock, flags);
