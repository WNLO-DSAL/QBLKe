...
{
	struct nvm_dev *nvmdev = qblk->dev->parent;
	char tsname[32];
	int i;
	int cpus = num_possible_cpus();

	for (i = 0; i < cpus; i++) {
		void **ctx = per_cpu_ptr(qblk->percpu_dmapool, i);

		sprintf(tsname, "rb_dmapool_%u", i);
		*ctx = nvmdev->ops->create_dma_pool(nvmdev, tsname);
		if (!*ctx)
			goto errout;
	}
	return 0;
errout:
	while (i--) {
		...
		nvmdev->ops->destroy_dma_pool(*ctx);
	}
	return -ENOMEM;
}
...
void qblk_destroy_percpu_dma(struct qblk *qblk) {
	...
	for_each_possible_cpu(cpu) {
		void **ctx = per_cpu_ptr(qblk->percpu_dmapool, cpu);
...
{
	struct nvm_dev *nvmdev = qblk->dev->parent;
	void **ctx = per_cpu_ptr(qblk->percpu_dmapool, cpuid);

	return nvmdev->ops->dev_dma_alloc(nvmdev, *ctx,
...
void *qblk_percpu_dma_alloc(struct qblk *qblk, int cpuid,
								gfp_t mem_flags, dma_addr_t *dma_handler)
{
	return nvm_dev_dma_alloc(qblk->dev->parent, mem_flags, dma_handler);
}

void qblk_percpu_dma_free(struct qblk *qblk, int cpuid, void *addr,
								dma_addr_t dma_handler)
{
	nvm_dev_dma_free(qblk->dev->parent, addr, dma_handler);
}
			...
			u64 offset_in_line)
{
	...
	if (line->state == QBLK_LINESTATE_CLOSED) {
		old_list = qblk_line_gc_list(qblk, chi, line);
		le32_add_cpu(line->vsc, -1);
		new_list = qblk_line_gc_list(qblk, chi, line);
...
struct qblk_line *qblk_line_get_data(struct ch_info *chi)
{
	...
	rcu_read_lock();
	line = rcu_dereference(chi->data_line);
	rcu_read_unlock();
	return line;
	...
	might_sleep();
retry:
	spin_lock_irqsave(&chi->free_lock, flags);
	if (chi->replacing) {
		//someone is replacing data line, we need to wait and retry
		spin_unlock_irqrestore(&chi->free_lock, flags);
		schedule();
		goto retry;
	}
	line = chi->data_line;
...
static void __qblk_line_put(struct qblk *qblk, struct qblk_line *line)
{
	...
	spin_lock_irqsave(&line->lock, flags);
#ifdef QBLKe_DEBUG
	qblk_debug_check_line_put(line);
#endif
...
static void qblk_stop_writes(struct qblk *qblk, struct ch_info *chi)
{
	lockdep_assert_held(&chi->free_lock);

	qblk->state = QBLK_STATE_STOPPING;
}
...
//retry_erase:
	...
	if (left_seblks) {
		...
		if (atomic_read(&newline->left_eblks)) {
			...
			//This should never happen because we use per channel remain sectors to make sure
			//that each channel doesn't accept more than it can receive.
	...
	if (qblk->state != QBLK_STATE_RUNNING) {
		...
		rcu_assign_pointer(chi->data_line, NULL);
		chi->data_next = NULL;
		goto out;
	}
...
//retry_setup:
	...
	spin_lock(&chi->free_lock);
	chi->replacing = 0;
	rcu_assign_pointer(chi->data_line, newline);
	...
	if (!chi->data_next) {
		 ...
		 * fail gracefully
		 */
		pr_err("Weird!,%s,code line=%d\n", __func__, __LINE__);
		qblk_printSInfo(qblk);
		qblk_stop_writes(qblk, chi);
		chi->data_next = NULL;
	} else {
...
retry:
	...
	if (line->left_msecs < nr_secs) {
		...
		//Now we replace it!
		prev_line = rcu_dereference(chi->data_line);
		spin_lock_irqsave(&prev_line->lock, flags);
		if (prev_line->left_msecs < nr_secs) {
			spin_unlock_irqrestore(&prev_line->lock, flags);
			//we need to replace the data line
			line = qblk_line_replace_data(qblk, chi, prev_line);
			if (!line) {
				/*
				  ...
				  unsigned int nr_secs)
{
	...
	for (lba = slba; lba < slba + nr_secs; lba++) {
		...
		ppa = qblk_trans_map_get(qblk, lba);
		       ...
		       struct qblk_line *gc_line, u64 paddr_gc)
{
	...
	BUG_ON(qblk_rb_pos_oob(qblk, rb, qblk_addr_to_cacheline(ppa_new)));
#endif
...
out:
	 ...
	 */
	if (qblk_trans_map_atomic_cmp_and_xchg(qblk, lba,
				ppa_gc, ppa_new))
			ret = 0;
#endif
			...
			struct qblk_rb *rb, sector_t lba, struct ppa_addr ppa)
{
	...
	/* Callers must ensure that the ppa points to a cache address */
	BUG_ON(!qblk_addr_in_cache(ppa));
	BUG_ON(qblk_rb_pos_oob(qblk, rb, qblk_addr_to_cacheline(ppa)));
#endif
...
/* Caller should free rqd->meta_list on their own. */
...
int qblk_submit_io_for_partialread(struct qblk *qblk, struct nvm_rq *rqd)
{
	...
	return nvm_submit_io_nowait(dev, rqd);
}
			 ...
			 void *emeta_buf)
{
	...
	//						&dma_meta_list);
	meta_list = qblk_percpu_dma_alloc(qblk, cpuid,
...
free_rqd_dma:
	//nvm_dev_dma_free(dev->parent, rqd.meta_list, rqd.dma_meta_list);
	qblk_percpu_dma_free(qblk, cpuid, meta_list, dma_meta_list);
	return ret;
}
			...
			struct bio *bio, int off, int nr_pages)
{
	...
	/* If the request contains [va--va--d--d--va]
	 * style entries, its possible that
	 * off + nr_pages != bio->bi_vcnt
	 * This is not a bug, we just need to
	 * free this fraction of pages.
	 */
...
retry:
	if (list_empty(&chi->free_list)) {
		...
		pr_err("qblk: no free lines, ch=%d remain=%u\n",
									chi->ch_index, pch_rl->remain_secs);
...
int qblk_line_get_first_data(struct qblk *qblk)
{
	...
	for (ch_idx = 0; ch_idx < qblk->nr_channels; ch_idx++) {
		...
		rcu_assign_pointer(chi->data_line, line);
...
void qblk_write_should_kick(struct qblk *qblk, int index, int force_kick)
{
	...
	unsigned int secs_avail = qblk_rb_read_count(qblk, qblk_get_rb_by_cpuid(qblk, index));
