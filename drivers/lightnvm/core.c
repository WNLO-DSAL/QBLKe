...
int nvm_submit_io_nowait(struct nvm_tgt_dev *tgt_dev, struct nvm_rq *rqd)
{
	struct nvm_dev *dev = tgt_dev->parent;
	int ret;

	if (!dev->ops->submit_io_nowait)
		return -ENODEV;
	...
	/* In case of error, fail with right address format */
	ret = dev->ops->submit_io_nowait(dev, rqd);
	if (ret)
		nvm_rq_dev_to_tgt(tgt_dev, rqd);
	return ret;
}
EXPORT_SYMBOL(nvm_submit_io_nowait);
